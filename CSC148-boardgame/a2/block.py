...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if len(self.children) != 0:
            ...
            par_x = position[0]
            par_y = position[1]
            self.children[0].position = (par_x + size, par_y)
            self.children[1].position = (par_x, par_y)
            self.children[2].position = (par_x, par_y + size)
            self.children[3].position = (par_x + size, par_y + size)
            for child in self.children:
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            for i in self.children:
                x = random.random()
                if x < math.exp(-0.25 * self.level) and \
                        i.level < self.max_depth:
                    i.smash()
                else:
                    i.colour = random.choice(COLOUR_LIST)
            return True

    def _smash_first(self) -> None:
        """Produce four children for a single block, colour is random.

        Precondition: The block does not have children.
        """
        for _ in range(4):
            new_block = Block((0, 0), self._child_size(),
                              random.choice(COLOUR_LIST),
                              self.level + 1, self.max_depth)
            new_block._parent = self
    ...
    def combine(self) -> bool:
        ...
        """
        if len(self.children) == 0 or (not self.level == self.max_depth - 1):
            return False
        else:
            if not self._check_majority()[0]:
                return False
            else:
                self.colour = self._check_majority()[1]
                self.children = []
                return True

    def _check_majority(self) -> Tuple[bool, Optional[Tuple[int, int, int]]]:
        """Return Tuple that the first element is bool that whether or not the
        ...
        if len(self.children) != 0:
            ...
            if max(check.values()) == 3 or max(check.values()) == 4:
                res = (True, key_max)
                return res
    ...
    def create_copy(self) -> Block:
        ...
        if len(self.children) == 0:
            ...
            res = Block(self.position, self.size, self.colour, self.level,
                        self.max_depth)
            for sub in self.children:
                res.children.append(sub.create_copy())
                sub._parent = res
            return res
