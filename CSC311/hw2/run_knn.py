...
def run_knn():
    ...
    predict_rates = []
    for k in k_values:
        predict_labels = knn(k, train_inputs, train_targets, valid_inputs)
        rate = len(predict_labels[predict_labels == valid_targets])/ len(predict_labels)
        predict_rates.append(rate)
    plt.plot(k_values, predict_rates, marker='o')
    plt.xlabel("k values")
    plt.ylabel("classification accuracy")
    plt.savefig("3.1a.png")
    
    optiaml_list = [3, 5, 7]
    for k in optiaml_list:
        test_pred = knn(k, train_inputs, train_targets, test_inputs)
        rate = len(test_pred[test_pred == test_targets]) / len(test_pred)
        print(k, rate)
