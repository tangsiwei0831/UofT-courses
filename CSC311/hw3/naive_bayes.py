...
def train_mle_estimator(train_images, train_labels):
    ...
    train_num = np.sum(train_labels, axis=0)
    pi_mle = train_num / len(train_labels)
    theta_mle = np.dot(train_images.transpose(), train_labels) / train_num
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    theta_map = (2 + np.dot(train_images.transpose(), train_labels)) / (4 + np.sum(train_labels, axis=0))
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    Bernoulli_likelihood = np.dot(images, np.log(theta)) + np.dot((1-images), np.log((1-theta)))
    sumsummation_part = np.log(pi) + Bernoulli_likelihood
    remove = np.log(np.dot(np.exp(Bernoulli_likelihood), pi)).reshape((len(Bernoulli_likelihood), 1))
    log_like = sumsummation_part - remove
    return log_like
